...
XDATA u8 usbcom_buf[64];
XDATA u8 _copy_buf[64];
XDATA char _buf_size = 0;
XDATA char* _head;
XDATA char _head_size = 0;

void setUartBuf(char buf_size, char* head, char head_size){
    _buf_size = buf_size;
    _head = head;
...
char _check_head(){
    u8 i;
    for(i=0;i<_head_size;i++){
        if(_head[i] != usbcom_buf[i])return 0;
    }
    return 1;
}
...
void* readBuf(){
    u8 i=0;
    if(_check_head()){
        for(i=0;i<64;i++)_copy_buf[i] = usbcom_buf[i];
        return _copy_buf;
    }
    else {return ((void*)0);}
}
