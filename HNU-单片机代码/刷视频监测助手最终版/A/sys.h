	      ...
	      a, 系统初始化MySTC_Init(),系统调度函数MySTC_OS(),加载用户回调函数SetEventCallBack()
				b, 若干可设置和触发回调函数的事件；
					          enumEventSys1mS       : 1mS定时到
										...
										enumEventUart2Rxd     : 串口2上收到一个符合格式定义的数据包事件。串口2：485上UART、485上ModBus协议（2022年5月2日新增）、或EXT上UART（由初始化确定）
...
enum event{enumEventSys1mS,                      //系统1mS事件
           ...
           enumEventUart1Rxd,                    //串口1上收到一个符合格式定义的事件
	         enumEventUart2Rxd,                    //串口2上收到一个符合格式定义（"数据包格式"、或"ModBus协议+数据包格式"）的事件
