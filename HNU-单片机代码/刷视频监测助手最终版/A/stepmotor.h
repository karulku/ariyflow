...
StepMotor用于STC-B板控制步进电机。共提供1个驱动函数、3个应用函数：
  ...
  （4）GetStepMotorStatus(char StepMotor) 获取指定步进电机状态
	...
	（5）StepLedPrint(unsigned char led, unsigned char run)  ： 控制步进电机4相（4个指示灯）开关    
             函数参数：led=1、2、3、4,分别对应4个Step指示灯，   
						           run=0：关灯  run=1：开灯 
             函数返回：（无）
编写：徐成（电话18008400450）   2021年4月16日设计，2025年3月24日更新
*/

#ifndef _StepMotor_H_			
#define _StepMotor_H_		

void StepMotorInit();										                              //  步进电机模块初始化
char SetStepMotor(char StepMotor,unsigned char speed ,int steps );     //  指定步进电机、按指定转动速度、转动指定步
																																									//  函数参数：StepMotor  指定步进电机，取值（enum StepMotorName中定义）
																																									//              enumStepMotor1：  SM 接口上的步进电机
																																									//              enumStepMotor2：  此时，用L0～L3四个LED模拟一个4相步进电机
																																									//              enumStepMotor3：  此时，用L4～L7四个LED模拟一个4相步进电机
																																									...
																																									//              enumSetStepMotorFail：调用失败（电机名不在指定范围，或speed=0,或调用时正在转动）
int EmStop(char StepMotor);                                            //  紧急停止指定步进电机转动
	                                                                                //  函数参数：StepMotor  指定步进电机(函数参数不对将返回0值)
																																									//  函数返回：剩余未转完的步数
unsigned char GetStepMotorStatus(char StepMotor);                      //  获取指定步进电机状态
	                                                                                //  函数参数：StepMotor  指定步进电机
																																									//  函数返回：enumStepMotorFree:自由
																																									//            enumStepMotorBusy,忙（正在转动）
																																									//            enumSetStepMotorFail：调用失败（步进电机名不在指定范围）
void StepLedPrint(unsigned char led, unsigned char run);             //led=1、2、3、4,分别对应4个Step指示灯，   run=0：关灯  run=1：开灯  
